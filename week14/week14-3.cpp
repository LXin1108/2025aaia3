//week14-3.cpp 學習計畫 第四題
//LeetCode 445. Add Two Numbers II
class Solution {
public:
    ListNode* myReverse(ListNode* head) { //自訂函式(剛寫過week4-2.cpp)
       if(head==nullptr || head->next==nullptr) return head; //終止條件
       ListNode* ans = myReverse(head->next); //函式呼叫函式
       head->next->next = head; //把前面,反過來後的最後面
    ...
    ListNode* addTwoNumbers(ListNode* l1, ListNode* l2) {
       ...
       ListNode* ans = myAddTwoNumbers(list1,list2); //呼叫上週的week13-??.cpp
       return myReverse(ans); //反過來
    }
    // 還缺myAddTwoNumbers()函式, 要把它寫出來
